...
<template>
  <SkyCard>
    <SectionTitle
      :title="t('components.meteor.yearCalendar.title')"
      :subtitle="t('components.meteor.yearCalendar.subtitle')"
    />
    ...
    <div class="mb-4 flex items-center justify-between gap-3">
      <button
        ...
        :aria-label="t('components.meteor.yearCalendar.prevYear')"
        @click="emit('prev')"
      >
        ←
      </button>
      ...
      <button
        type="button"
        class="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm font-medium text-slate-200 transition hover:border-slate-500 hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500/40"
        :aria-label="t('components.meteor.yearCalendar.nextYear')"
        @click="emit('next')"
      >
        →
      </button>
      ...
      >
        ...
        >
          ...
          <div class="mt-1.5 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400 sm:text-sm">
            ...
            <span>{{ difficultyLabel(event.difficulty) }}</span>
          </div>
        </button>
      </li>
    </ul>
    ...
    >
      {{ t('components.meteor.yearCalendar.empty') }}
    </p>
  </SkyCard>
</template>
